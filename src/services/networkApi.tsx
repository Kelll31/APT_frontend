...
class NetworkApiService {
    ...
    async exportTopology(format: 'json' | 'svg' | 'png' | 'graphml'): Promise<Blob> {
        const response = await api.get<Blob>(`/api/network/topology/export?format=${format}`, {
            responseType: 'blob',
            headers: {
    ...
    async exportNetworkData(
        ...
        // ИСПРАВЛЕНИЕ: Явно указываем тип Blob
        const response = await api.post<Blob>('/api/network/export',
            { format, ...options },
            {
                responseType: 'blob',
                headers: {
    ...
    async importNetworkData(
        ...
        return api.post('/api/network/import', formData, {
            headers: {},
            timeout: 300000
        });
    }
    ...
    cidrToIpRange(cidr: string): { start: string; end: string; count: number } {
        ...
        // Проверяем, что оба значения определены
        ...
        // Безопасное разделение IP адреса с валидацией
        const networkParts = network.split('.').map(Number);
        if (networkParts.length !== 4 || networkParts.some(part => isNaN(part) || part < 0 || part > 255)) {
            throw new Error('Invalid IP address format in CIDR');
        }

        const hostBits = 32 - prefix;
        const hostCount = Math.pow(2, hostBits);

        // Вычисление с гарантированно определенными значениями
