...
class EnterpriseScannerApiService {
     ...
     */
    async startEnterpriseScan(settings: EnterpriseScanSettings): Promise<ApiResponse<{ scan_id: string; status: string; message: string; estimated_duration?: number; }>> {
     ...
     */
    async getEnterpriseScanStatus(scanId: string): Promise<ApiResponse<EnterpriseScanResult>> {
        if (!scanId) {
            throw new Error('Scan ID is required');
        }
        return api.get(`/api/enterprise/scanner/${encodeURIComponent(scanId)}/status`);
    }
    ...
    ): Promise<ApiResponse<EnterpriseScanResult>> {
        if (!scanId) {
            throw new Error('Scan ID is required');
        }
     ...
     */
    async startBatchScan(configuration: BatchScanConfiguration): Promise<ApiResponse<{ batch_id: string; status: string; message: string; }>> {
     ...
     */
    async getBatchScanStatus(batchId: string): Promise<ApiResponse<{
        batch_id: string;
     ...
     */
    async createScanSchedule(settings: ScanScheduleSettings): Promise<ApiResponse<{ schedule_id: string; message: string; }>> {
     ...
     */
    async updateScanSchedule(scheduleId: string, updates: Partial<ScanScheduleSettings>): Promise<ApiResponse<{ message: string; }>> {
        if (!scheduleId) {
     ...
     */
    async deleteScanSchedule(scheduleId: string): Promise<ApiResponse<{ message: string; }>> {
        if (!scheduleId) {
            throw new Error('Schedule ID is required');
        }
    ...
    async validateEnterpriseTarget(
        target: string,
        options: {
            deep_validation?: boolean | undefined;
            business_context?: BusinessScanContext | undefined;
            compliance_check?: boolean | undefined;
            threat_intelligence?: boolean | undefined;
            permission_check?: boolean | undefined;
        } = {}
    ): Promise<ApiResponse<{
        valid: boolean;
    ...
    async createScanTemplate(template: {
        ...
        compliance_frameworks?: string[];
        tags?: string[];
    }): Promise<ApiResponse<{ template_id: string; message: string; }>> {
     ...
     */
    async updateScanTemplate(templateId: string, updates: Record<string, any>): Promise<ApiResponse<{ message: string; }>> {
        if (!templateId) {
     ...
     */
    async deleteScanTemplate(templateId: string): Promise<ApiResponse<{ message: string; }>> {
        if (!templateId) {
            throw new Error('Template ID is required');
        }
     ...
     */
    async exportScanData(exportParams: {
        scan_ids?: string[];
        ...
        const response = await api.post('/api/enterprise/scanner/export', exportParams, {
            headers: {
    ...
    ): Promise<ApiResponse<{ scan_id: string; message: string; }>> {
        if (!scanId) {
            throw new Error('Scan ID is required');
        }
     ...
     */
    async getScannerConfiguration(): Promise<ApiResponse<{
        global_settings: {
            max_concurrent_scans: number;
     ...
     */
    async updateScannerConfiguration(updates: {
     ...
     */
    async syncWithExternalSystems(syncConfig: {
        systems: Array<{
     ...
     */
    async trainMLModels(trainingConfig: {
        model_types: string[];
     ...
     */
    async getMLTrainingStatus(jobId: string): Promise<ApiResponse<{
        job_id: string;
    ...
    private validatePortRange(portRange: string): boolean {
        ...
        for (const part of parts) {
            if (part.includes('-')) {
                ...
                // ИСПРАВЛЕНИЕ: Добавляем проверки на undefined
    ...
    getEnterpriseApiStats(): {
        ...
        // ИСПРАВЛЕНИЕ: Возвращаем правильный тип с всеми требуемыми свойствами
        return {
            totalRequests: baseStats.totalRequests || 0,
            scanRequests: 0, // Эти метрики будут собираться отдельно
            templateRequests: 0,
            agentRequests: 0,
            analyticsRequests: 0,
            mlRequests: 0,
            cacheHitRatio: baseStats.cacheHitRatio || 0,
            averageResponseTime: baseStats.averageResponseTime || 0
        };
    }
    ...
    async checkEnterpriseFeatures(): Promise<{
        ...
        try {
            // ИСПРАВЛЕНИЕ: Правильная типизация response
            ...
            }>('/api/enterprise/scanner/features');
    ...
    async exportConfiguration(options: {
        ...
        // ИСПРАВЛЕНИЕ: Используем публичные методы вместо приватного request
        const response = await api.post('/api/enterprise/scanner/configuration/export', options, {
            headers: {
    ...
    ): Promise<ApiResponse<{ message: string; imported_items: number; warnings: string[]; }>> {
        const formData = new FormData();
        formData.append('file', file);
        formData.append('options', JSON.stringify(options));

        // ИСПРАВЛЕНИЕ: Используем публичные методы вместо приватного request
        return api.post('/api/enterprise/scanner/configuration/import', formData, {
