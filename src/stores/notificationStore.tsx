...
export interface NotificationActions {
    ...
    initialize: () => void;
    cleanup: () => void;
    applyNotificationRules: (notification: Notification) => void;
    findDuplicateNotification: (notification: Notification) => string | null;
    handleDuplicateNotification: (existingId: string, newNotification: Notification) => string;
    playNotificationSound: (sound: string | boolean) => void;
    triggerVibration: () => void;
    shouldArchive: (notification: Notification) => boolean;
    setActionLoading: (notificationId: string, actionId: string, loading: boolean) => void;
    showConfirmation: (message: string) => Promise<boolean>;
    processStringAction: (action: string, notification: Notification) => Promise<void>;
    replaceVariables: (template: string, variables: Record<string, any>) => string;
    calculateStatistics: (notifications: Notification[]) => NotificationStats;
    initializeDefaultGroups: () => void;
    initializeDefaultTemplates: () => void;
    startCleanupTimer: () => void;
    evaluateRuleConditions: (conditions: any[], notification: Notification) => boolean;
    applyRuleActions: (actions: any[], notification: Notification) => void;
    getNestedValue: (obj: any, path: string) => any;
    evaluateCondition: (value: any, operator: string, expected: any, caseSensitive?: boolean) => boolean;
    performCleanup: () => void;
}
        ...
        subscribeWithSelector((set, get) => ({
            ...
            executeAction: async (notificationId: string, actionId: string) => {
                ...
                try {
                    ...
                    // ИСПРАВЛЕНИЕ: правильная обработка типа unknown
                    const errorMessage = error instanceof Error ? error.message : 'Неизвестная ошибка';
                    get().createNotification('error', 'Ошибка', `Не удалось выполнить действие: ${errorMessage}`);
                } finally {
