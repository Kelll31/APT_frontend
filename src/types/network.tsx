...
// Добавляем недостающий интерфейс NetworkSecurityInfo
...
export interface SpanningTreeInfo {
    ...
    bridgeId: string;
    rootBridge: boolean;
    portStates: Record<string, string>;
}
...
// Безопасные функции-утилиты для работы со строками
...
const safeSplit = (str: string, separator: string, index: number): string => {
...
export const isPrivateIP = (ip: string): boolean => {
    // Безопасная проверка для наиболее распространенных диапазонов
    if (!ip || typeof ip !== 'string') return false;

    return ip.startsWith('10.') ||
        ip.startsWith('192.168.') ||
        (ip.startsWith('172.') &&
            safeParseInt(safeSplit(ip, '.', 1)) >= 16 &&
            safeParseInt(safeSplit(ip, '.', 1)) <= 31) ||
        ip.startsWith('127.');
};
...
export const calculateNetworkSize = (cidr: string): number => {
    ...
    const prefixLength = safeParseInt(safeSplit(cidr, '/', 1));
...
export const ipToNumber = (ip: string): number => {
    ...
    return ip.split('.').reduce((acc, octet) => acc * 256 + parseInt(octet || '0', 10), 0);
};
...
export const isIPInRange = (ip: string, cidr: string): boolean => {
    ...
    const network = safeSplit(cidr, '/', 0);
    const prefixLengthStr = safeSplit(cidr, '/', 1);

    if (!network || !prefixLengthStr) return false;

    const prefixLength = safeParseInt(prefixLengthStr);
