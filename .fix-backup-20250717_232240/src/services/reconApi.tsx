 ...
 */
export class EnterpriseReconAPI {
    ...
    async getNetworkInterfaces(): Promise<ApiResponse<NetworkInterface[]>> {
        try {
            const response = await this.apiClient.get<NetworkInterface[]>('/api/network/interfaces');
            return {
                success: true,
                data: response.data,
                message: 'Сетевые интерфейсы получены успешно'
            };
        } catch (error) {
            return {
                success: false,
                error: error.message,
                message: 'Не удалось получить сетевые интерфейсы'
            };
        }
    }
     ...
     */
    async startEnterpriseRecon(settings: EnterpriseReconSettings): Promise<ApiResponse<{ scan_id: string }>> {
        try {
            const response = await this.apiClient.post<{ scan_id: string }>(
                `${this.enterpriseEndpoint}/start`,
                settings
            );
            ...
            return {
                success: true,
                data: response.data,
                message: 'Enterprise разведка запущена успешно'
            };
        } catch (error) {
            return {
                success: false,
                error: error.message,
                message: 'Не удалось запустить enterprise разведку'
            };
        }
    }
     ...
     */
    async stopEnterpriseRecon(scanId: string): Promise<ApiResponse<void>> {
        try {
            await this.apiClient.post(`${this.enterpriseEndpoint}/stop/${scanId}`);

            return {
                success: true,
                message: 'Enterprise разведка остановлена успешно'
            };
        } catch (error) {
            return {
                success: false,
                error: error.message,
                message: 'Не удалось остановить enterprise разведку'
            };
        }
    }
     ...
     */
    async getEnterpriseReconResults(scanId: string): Promise<ApiResponse<EnterpriseReconResult>> {
        try {
            const response = await this.apiClient.get<EnterpriseReconResult>(
                `${this.enterpriseEndpoint}/results/${scanId}`
            );

            return {
                success: true,
                data: response.data,
                message: 'Результаты enterprise разведки получены успешно'
            };
        } catch (error) {
            return {
                success: false,
                error: error.message,
                message: 'Не удалось получить результаты enterprise разведки'
            };
        }
    }
    ...
    async getThreatIntelligence(filters?: {
        ...
        dateFrom?: string;
        dateTo?: string;
    }): Promise<ApiResponse<ThreatIntelligenceData>> {
        try {
            const response = await this.apiClient.get<ThreatIntelligenceData>(
                `${this.enterpriseEndpoint}/threat-intelligence`,
                { params: filters }
            );
            ...
            return {
                success: true,
                data: response.data,
                message: 'Данные threat intelligence получены успешно'
            };
        } catch (error) {
            return {
                success: false,
                error: error.message,
                message: 'Не удалось получить данные threat intelligence'
            };
        }
    }
    ...
    async getComplianceResults(checkId: string): Promise<ApiResponse<ComplianceResults>> {
        try {
            const response = await this.apiClient.get<ComplianceResults>(
                `${this.enterpriseEndpoint}/compliance/results/${checkId}`
            );

            return {
                success: true,
                data: response.data,
                message: 'Результаты соответствия получены успешно'
            };
        } catch (error) {
            return {
                success: false,
                error: error.message,
                message: 'Не удалось получить результаты соответствия'
            };
        }
    }
    ...
    async getIntegrations(): Promise<ApiResponse<EnterpriseIntegration[]>> {
        try {
            const response = await this.apiClient.get<EnterpriseIntegration[]>(
                `${this.enterpriseEndpoint}/integrations`
            );

            return {
                success: true,
                data: response.data,
                message: 'Интеграции получены успешно'
            };
        } catch (error) {
            return {
                success: false,
                error: error.message,
                message: 'Не удалось получить интеграции'
            };
        }
    }
    ...
    async testIntegration(integrationId: string): Promise<ApiResponse<{ status: string; message: string }>> {
        try {
            const response = await this.apiClient.post<{ status: string; message: string }>(
                `${this.enterpriseEndpoint}/integrations/${integrationId}/test`
            );
    ...
    async getAssetInventory(
        ...
        try {
            const response = await this.apiClient.get<EnterpriseNetworkDevice[]>(
                `${this.enterpriseEndpoint}/assets/inventory`,
                { params: filters }
            );
    ...
    async updateAssetClassification(
        ...
        try {
            await this.apiClient.put(
                `${this.enterpriseEndpoint}/assets/${assetId}/classification`,
                classification
            );
    ...
    async getReportStatus(reportId: string): Promise<ApiResponse<{ status: string; progress: number }>> {
        try {
            const response = await this.apiClient.get<{ status: string; progress: number }>(
                `${this.enterpriseEndpoint}/reports/status/${reportId}`
            );
    ...
    connectWebSocket(scanId: string): void {
        ...
        const wsUrl = `ws://${window.location.host}/api/enterprise/recon/ws/${scanId}`;
...
export const reconApi = new EnterpriseReconAPI(api);
...
export function validatePortRange(range: string): boolean {
    ...
    // Смешанный формат: 80,443,8000-8999
    const mixedRegex = /^(\d+|\d+-\d+)(,\s*(\d+|\d+-\d+))*$/;

    if (!mixedRegex.test(range.replace(/\s/g, ''))) {
        return false;
    }

    // Проверяем валидность номеров портов
...
export function importSettingsFromJSON(json: string): EnterpriseReconSettings {
    try {
        ...
        throw new Error(`Ошибка импорта настроек: ${error.message}`);
    }
}
