...
class EnterpriseReportsApiService {
    ...
    async generateExecutiveReport(
        ...
        try {
            ...
            if (error.response?.status === 403) {
                ...
                throw new Error(`Executive report generation failed: ${error.message || 'Unknown error'}`);
            }
        }
    }
    ...
    private async validateDataAvailability(reportConfig: any): Promise<{
        ...
        try {
            const response = await api.post('/api/enterprise/reports/data-availability', {
     ...
     */
    private calculateValidityPeriod(reportType: string, timeframe: string): string {
    ...
    private async initializeApprovalWorkflow(
        ...
        try {
            const response = await api.post('/api/enterprise/reports/approval/initialize', {
                report_id: reportId,
     ...
     */
    async getExecutiveReportStatus(generationId: string): Promise<ApiResponse<{
        generation_id: string;
        ...
        try {
            ...
            const response = await api.get(`/api/enterprise/reports/executive/generation/${encodeURIComponent(generationId)}/status`);
            ...
            throw new Error(`Failed to retrieve report status: ${error.message}`);
        }
    }
    ...
    async downloadExecutiveReport(
        ...
        try {
            ...
            const response = await api.get(`/api/enterprise/reports/executive/${encodeURIComponent(reportId)}/download?${queryParams}`, {
                headers: {
